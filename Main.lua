...
maps:Button({
	Name = "Tower";
	Callback = function()
		loadbuild(loadstring(game:HttpGet(""))())
	end
})
...
ats:Toggle({
	...
	Enabled = false,
	Callback = function(b)
		settings["loop kill"] = b
		while true do wait()
			if settings["loop kill"] then
				killplayer(target)
			end
		end
	end
...
ats:Toggle({
	...
	Enabled = false,
	Callback = function(b)
		settings["loop tools"] = b
		while true do wait()
			if settings["loop tools"] then
				removetools(target)
			end
		end
	end
